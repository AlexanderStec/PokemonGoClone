...
let map = new InteractiveMap({
	...
	initializeMap() {
		...
		this.loadLandmarks("landmarks-natural-evanston", (landmark) => {
			...
			return landmark.properties.tree
		})
		...
		for (var i = 0; i < 50; i++) {
			...
			let position = clonePolarOffset(NU_CENTER, 600*Math.random() + 200, 20*Math.random())
			this.createLandmark({
				pos: position,
	...
	update() {
		if (gameState.time > 0){
			gameState.time = Math.round((gameState.time-.1) * 10) / 10
		}
	},
